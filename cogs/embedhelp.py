...
class Help:
    ...
    async def _help(self, ctx, command = None):
        ...
        if not command:
            ...
            if toggle == "dm":
                await self.bot.say("Check your DM, {}! I have sent something "
                                   "for you :wink:".format(author.mention))
                for em in to_send:
                    await self.bot.send_message(ctx.message.author,
                                                embed=em)
                await self.bot.send_message(ctx.message.author,
                                        "Join the support server for help: <https://discord.gg/KS4a4yQ>"
                                        "We also have Nadeko bot integrated into it, type ``.help`` for info. For info contact Mr. Robot#0007")
            elif toggle == 'no_dm':
                for em in to_send:
                    await self.bot.say(embed=em)
                await self.bot.say("Join the support server for help: <https://discord.gg/KS4a4yQ>"
                                   ". We also have Nadeko bot integrated into it, type ``.help`` for info. For info contact Mr. Robot#0007")
