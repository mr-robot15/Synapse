...
class Owner:
    ...
    async def _set(self, ctx):
        """Changes Synapse's's core settings"""
    ...
    async def owner(self, ctx):
        ...
        if self.setowner_lock:
            await self.bot.say("Sorry for inconvenience!")
            return

        if self.bot.settings.owner is not None:
            await self.bot.say(
            "You cannot set the owner. Changing the owner might lead to the "
            "bot getting deleted and can cause fatal errors in bot."
            "We have removed this command for security reasons!"
            )
            await asyncio.sleep(3)

        await self.bot.say("You can still use the other commands")
    ...
    async def prefix(self, ctx, *prefixes):
        """Sets Synapse's global prefixes
    ...
    async def serverprefix(self, ctx, *prefixes):
        """Sets Synapse's prefixes for this server
    ...
    async def stream(self, ctx, streamer=None, *, stream_title=None):
        """Sets Synapse's streaming status
    ...
    async def _whitelist_add(self, user: GlobalUser):
        """Adds user to Synapse's global whitelist"""
    ...
    async def shutdown(self, silently : bool=False):
        ...
        try: # We don't want missing perms to stop our shutdown
            if not silently:
                await self.bot.say("Shutting down. Bye Mr. Robot! " + wave + skin)
        except:
            pass
    ...
    async def restart(self, silently : bool=False):
        ...
        try:
            if not silently:
                await self.bot.say("Restarting. Will be back Mr. Robot!")
        except:
            pass
    ...
    @checks.is_owner()
    async def join(self):
        """Shows Synapse's invite URL"""
        if self.bot.user.bot:
            await self.bot.whisper("Invite URL: " + "https://discordapp.com/oauth2/authorize?client_id=435538622293344266&scope=bot&permissions=8")
        else:
    ...
    async def info(self):
        ...
        about = (
            "This is a bot coded by Mr. Robot, wave him! "
            "For any queries DM Mr. Robot#0007\n\n"
            "He will help you in any situation no matter what. "
            "Synapse is made to be good for everything just type !help  "
            "and see the magic\n\n"
            "".format(red_repo, author_repo, server_url))

        embed = discord.Embed(colour=discord.Colour.red())
        embed.add_field(name="Created by", value=str(owner))
        embed.add_field(name="Python", value=py_version)
        embed.add_field(name="discord.py", value=dpy_version)
        embed.add_field(name="About Synapse", value=about, inline=False)
        embed.set_footer(text="Bringing joy since 16 April 2018 (over "
    ...
    async def version(self):
        """Shows Synapse's current version"""
    ...
    def _wait_for_answer(self, author):
        ...
        if choice == "yes":
            print(author.name + " this cannot be done!")
            self.setowner_lock = False
        else:
    ...
    def _get_version(self):
        if not os.path.isdir(".git"):
            msg = "This instance of Synapse hasn't been installed with git."
